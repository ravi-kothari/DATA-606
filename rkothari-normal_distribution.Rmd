...
```
The two distributions both are near normal which are resulting in a bell shape. The men's heights are slight skewing to the right while the women's to the left.
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    ...
    Yes, the data is following a nearly normal distribution. Both women and men's curves are bell shaped with the   exception with the men's curve stopping at 190.
...
The plot for sim_norm appears to be more of a straight line when compared to the original data. The original data has some peaks in quantiles 1 and 2. For the most part, the two plots are comparable.
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    The fdims$hgt plot looks similar to the simulated data. The data indicates that the female heights are nearly normal.
...
qqnormsim(fdims$wgt)
```
For the most part, female weight appears to be nearly normal with the exception of some outliers.
...
6.  Write out two probability questions that you would like to answer; one 
    ...
    How many females are shorter than 5'0?
    ```{r eval=TRUE}
pnorm(q=150.24, mean = fhgtmean, sd = fhgtsd )
sum(fdims$hgt < 150.24) / length(fdims$hgt)
```
    How many females weigh more than 130lbs?
```{r eval=TRUE}
pnorm(q=59, mean = fwgtmean, sd=fwgtsd)
sum(fdims$wgt > 59) / length(fdims$wgt)
```

The height variable had a closer agreement between the empirical and normal distribution.
* * *
...
```{r}
female_body <- function(data){
par(mfrow = c(1, 2))
hist(data, probability = TRUE)
qqnorm(data)
qqline(data)
}
female_body(fdims$bii.di)
```
...
```{r}
female_body(fdims$kne.di)
```
The knee diameter histogram is appearing to be right skewed - the qqplot is demonstrating the same.
