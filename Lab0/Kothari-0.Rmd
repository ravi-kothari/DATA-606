...
source("~/MSDA/DATA 606/Lab0/more/arbuthnot.r")
```
...
```{r plot-propotion_boys-vs-year, eval=FALSE}
...
source("~/MSDA/DATA 606/Lab0/more/present.R")
```
...
```{r view-data_present, eval=FALSE}
present
```

-   What are the dimensions of the data frame

```{r view-dim_present, eval=FALSE}
dim(present)
```
...
```{r view_names_present, eval=FALSE}
names(present)
```
...
```{r counts_present, eval=FALSE}
present$boys
present$girls
```

The counts in the Present US dataset is in millions compared to the count in the Arbuthnot's dataset which was in thousands.Also the US data has 63 rows so the data is from 1940 to 2002 whereas in Arbuthnot's dataset there were 82 rows so from year 1629 to 1710.
...
-   As we can see from the ratio below that its greater than one for all the years, it means boys being born in greater             proportion which was true in the case of Arbuthnot's observation as well.

```{r boys_girls_ratio, eval=FALSE}
present$boys/present$girls
```

-   Plot of the ratio 
```{r plot-boys_girls_ratio-vs-year, eval=FALSE}
...
-   In what year did we see the most total number of births in the U.S.? You can
    ...
    Total number of birth can be calculated by adding the number of boys and girls and by using the max function we can see that     in 1961 the total number of birth was highest.
    
```{r most_total_birth, eval=FALSE}
